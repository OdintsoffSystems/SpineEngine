...
public class TiledLayer extends Layer {
     ...
     */
    public TiledLayer(Game game, int columns, int rows, Image image, int tileWidth,
		      int tileHeight) {
	...
	// in the Layer super-class constructor
        ...
        if (((image.getWidth(null) % tileWidth) != 0) ||
            ((image.getHeight(null) % tileHeight) != 0)) {
	...
	this.rows = rows;
        ...
        int noOfFrames =
            (image.getWidth(null) / tileWidth) * (image.getHeight(null) / tileHeight);
    ...
    public void setStaticTileSet(Image image, int tileWidth, int tileHeight) {
	    ...
	    ((image.getWidth(null) % tileWidth) != 0) ||
            ((image.getHeight(null) % tileHeight) != 0)) {
             throw new IllegalArgumentException();
	}
    ...
    public final void draw() {
        ...
        Graphics g = game.getOffscreenBufferGraphics();
	    ...
	    int endRow = this.rows;
            Rectangle gRect = new Rectangle(game.getGameResolution());

	    // calculate the number of columns left of the clip
	    int number = (gRect.x - this.x) / cellWidth;
	    if (number > 0) {
		startColumn = number;
	    }
	    ...
	    int endClipX = gRect.x + gRect.width;
	    ...
	    number = (gRect.y - this.y) / cellHeight;
	    if (number > 0) {
		startRow = number;
	    }
	    ...
	    int endClipY = gRect.y + gRect.height;
		 ...
		 row < endRow; row++, ty += cellHeight) {
			...
			continue;
                    ...
                    g.drawImage(sourceImage,
                                ...
                                tileSetX[tileIndex],
                                tileSetY[tileIndex],
                                tileSetX[tileIndex] + cellWidth,
                                tileSetY[tileIndex] + cellHeight,
                                null);
                }
            }
	}
